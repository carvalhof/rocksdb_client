...
void fill_tcp_packet(tcp_control_block_t *block, struct rte_mbuf *pkt, uint32_t frame_size) {
	// ensure that IP/TCP checksum offloadings
	pkt->ol_flags |= (RTE_MBUF_F_TX_IPV4 | RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_TCP_CKSUM);
	...
	uint32_t tcp_payload_size = frame_size - sizeof(struct rte_ether_hdr) - sizeof(struct rte_ipv4_hdr) - sizeof(struct rte_tcp_hdr);
