...
SERVER_LAYOUT_LIST=("dispatcher1" "dispatcher2" "dispatcher4")
...
for l in ${SERVER_LAYOUT_LIST[@]}; do
    ...
    for j in `seq 0 $RUNS`; do
        ...
        LD_LIBRARY_PATH=${CLIENT_ROCKSDB_DIR} timeout $(( SERVER_TIMEOUT + 10 )) ./client -h ${CLIENT_SERVER_ADDR} -p ${CLIENT_SERVER_PORT} -n ${CLIENT_REQUESTS} -c ${CLIENT_CONNECTIONS} -t ${SERVER_NUMBER_OF_THREADS} -o ${CLIENT_OUTPUT_FILE} -s ${SEED} -w ${CLIENT_WARMINGUP} -l ${CLIENT_CORES} -f -m ${CLIENT_PROPORTION} 1>/dev/null 2>/dev/null &
