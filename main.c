...
uint16_t *flow_indexes_for_warming;
uint32_t *interarrival_gap;
application_node_t *warming_array;
...
int process_rx_pkt(struct rte_mbuf *pkt, node_t *incoming, uint32_t *incoming_idx) {
	...
	if(t0 == -1) {
		// It was packet from the Warming up phase... so discard
		return 0;
	}
...
static void start_warming_up(uint16_t portid, uint8_t qid) {
	struct rte_mbuf *pkt;

	uint64_t warming_gap = 1000 * TICKS_PER_US; //1ms

	uint64_t next_tsc = rte_rdtsc() + warming_gap;

	for(uint64_t i = 0; i < nr_warming; i++) {
		// allocated the packet
		pkt = rte_pktmbuf_alloc(pktmbuf_pool_tx);

		// choose the flow to send
		uint16_t flow_id = flow_indexes_for_warming[i];
		tcp_control_block_t *block = &tcp_control_blocks[flow_id];
		...
		// fill the payload
		fill_payload_pkt(pkt, 0, -1);
		fill_warming_data(pkt, i);
...
int main(int argc, char **argv) {
	...
	// create warming up array
	create_warming_array();
